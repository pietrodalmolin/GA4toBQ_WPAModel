INSERT INTO `steam-mantis-108908.WPA_Events.SB_SubCategoryClick`
(
SELECT
...
WHEN STRPOS(interface_component, ' - ') > 0 THEN 
    ARRAY_REVERSE(
        SPLIT(SUBSTR(interface_component, STRPOS(interface_component, ' - ') + 3), '.')
    )[OFFSET(0)]
ELSE 
    NULL
...
AND EventAction LIKE 'SubCategory Click%' --filter event_action
