INSERT INTO `steam-mantis-108908.WPA_Events.GX_CrossSellingWidget`
 (
SELECT
...
,Interface_WidgetName
,Interface_Component
--CUSTOM DIMENSIONS
,CASE
WHEN STRPOS(LOWER(page_location), 'http') > 0 THEN
-- Full URL case
CASE
WHEN SPLIT(page_location, '/')[SAFE_OFFSET(3)] IS NULL OR SPLIT(page_location, '/')[SAFE_OFFSET(3)] = '' THEN 'Market Not Specified: Brand Page'
WHEN LENGTH(SPLIT(page_location, '/')[SAFE_OFFSET(3)]) > 2 THEN 'Market Not Specified: Brand Page'
ELSE SPLIT(page_location, '/')[SAFE_OFFSET(3)]
END
ELSE
-- Relative path case
CASE
WHEN SPLIT(page_location, '/')[SAFE_OFFSET(1)] IS NULL OR SPLIT(page_location, '/')[SAFE_OFFSET(1)] = '' THEN 'Market Not Specified: Brand Page'
WHEN LENGTH(SPLIT(page_location, '/')[SAFE_OFFSET(1)]) > 2 THEN 'Market Not Specified: Brand Page'
ELSE SPLIT(page_location, '/')[SAFE_OFFSET(1)]
END
END AS Market
--CUSTOM DIMENSIONS
...
-- EVENT FILTERS
AND Event_Name IN ('Clicks', 'Games', 'Feature')
AND Content_Group = 'SPORTSBOOK'
AND Interface_SectionName = 'Cross_Selling_Widget'
AND (EventAction NOT LIKE '%For Fun%' OR EventAction IS NULL)
AND (Interface_WidgetName NOT LIKE '%Game Launcher%' OR Interface_WidgetName IS NULL)

GROUP BY
--SESSION LEVEL COLUMNS
property_id,date,FARM_FINGERPRINT(CONCAT(wpa.user_pseudo_id, ga_session_id)),FARM_FINGERPRINT(wpa.user_pseudo_id),GUID_Event,
GUID_User,ga_session_number,session_engaged,Interface_Brand,Customer_Status_Start,geo.city,geo.country,geo.region,device.mobile_brand_name,device.web_info.browser,device.web_info.browser_version,device.category,device.mobile_marketing_name,device.mobile_model_name,device.operating_system,device.operating_system_version
--GLOBAL DIMENSIONS
